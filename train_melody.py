...
from articulation_encoder import clear_articulation_encoder_cache
from melody_inference import predict_outputs, decode_prediction
from melody_evaluation import (song_split, note_counts, summarize_note_counts, file_digest,
    semitone_aba_counts, passes_pitch_aba_guard, note_artifact_counts, passes_artifact_guard)
from musical_boundary import boundary_targets
...
def main() -> None:
    ...
    source_provenance = {p.name: file_digest(p) for p in Path(__file__).resolve().parent.glob("*.py")}
    for sid in ids:
